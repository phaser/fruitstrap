...
# TODO: Fix the new testing stuff (added as separate task)
#if (TESTS)
#    ENABLE_TESTING()
#endif (TESTS)
...
set (GNU_CXX_FLAGS_DEBUG "${GNU_GPROF_FLAGS} ${CUSTOM_COMPILER_OPTS} -g -O0 -W -Wall -DBUILD_TYPE_DEBUG")
set (GNU_C_FLAGS_DEBUG "${GNU_GPROF_FLAGS} ${CUSTOM_COMPILER_OPTS} -g -O0 -W -Wall -DBUILD_TYPE_DEBUG")
set (GNU_CXX_FLAGS_RELEASE "-Os ${CUSTOM_COMPILER_OPTS}")
set (GNU_C_FLAGS_RELEASE "-Os ${CUSTOM_COMPILER_OPTS}")

if (ANDROID)
	if (CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX)
	        set (CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS} ${GNU_CXX_FLAGS_DEBUG} -std=gnu++0x")
        	set (CMAKE_C_FLAGS_DEBUG "${CMAKE_C_FLAGS} ${GNU_C_FLAGS_DEBUG}")
	        set (CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS} ${GNU_CXX_FLAGS_RELEASE} -std=gnu++0x")
        	set (CMAKE_C_FLAGS_RELEASE "${CMAKE_C_FLAGS} ${GNU_C_FLAGS_RELEASE}")
	endif (CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX) 
elseif (LINUX)
   	if (CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX)
		set (CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS} ${GNU_CXX_FLAGS_DEBUG}")
       	set (CMAKE_C_FLAGS_DEBUG "${CMAKE_C_FLAGS} ${GNU_C_FLAGS_DEBUG}")
		set (CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS} ${GNU_CXX_FLAGS_RELEASE}")
       	set (CMAKE_C_FLAGS_RELEASE "${CMAKE_C_FLAGS} ${GNU_C_FLAGS_RELEASE}")
	endif (CMAKE_COMPILER_IS_GNUCC OR CMAKE_COMPILER_IS_GNUCXX) 
elseif (WIN32)
	configure_msvc_runtime()
	set (CMAKE_CXX_FLAGS "/EHsc /W4 /MP /D_CRT_SECURE_NO_WARNINGS /wd4201 /wd4512 /DWIN32 /D_WINDOWS ${CUSTOM_COMPILER_OPTS}")
	set (CMAKE_C_FLAGS "/EHsc /W3 /MP /D_CRT_SECURE_NO_WARNINGS ${CUSTOM_COMPILER_OPTS}")
elseif (IOS)
    set (CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS} ${GNU_CXX_FLAGS_DEBUG}")
    set (CMAKE_C_FLAGS_DEBUG "${CMAKE_C_FLAGS} ${GNU_C_FLAGS_DEBUG}")
    set (CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS} ${GNU_CXX_FLAGS_RELEASE}")
